<div class="btn-group dropleft">
    ...
    <div class="dropdown-menu">
        {{-- @can('access_purchase_payments') --}}
            <a href="{{ route('hajj-saving-payments.index', $data->id) }}" class="dropdown-item">
                <i class="bi bi-cash-coin mr-2 text-warning" style="line-height: 1;"></i> Show Payments
            </a>
        {{-- @endcan --}}
        {{-- @can('access_purchase_payments') --}}
            {{-- @if($data->due_amount > 0) --}}
                <a href="{{ route('hajj-saving-payments.create', $data->id) }}" class="dropdown-item">
        ...
        {{-- @endcan
        @can('show_purchases') --}}
            <a href="{{ route('hajj-savings.show', $data->id) }}" class="dropdown-item">
        ...
        @can('delete_purchases') --}}
            <button id="delete" class="dropdown-item" onclick="
                ...
                <form id="destroy{{ $data->id }}" class="d-none" action="{{ route('hajj-savings.destroy', $data->id) }}" method="POST">
                    @csrf
                    @method('delete')
                </form>
            </button>
